...
3. Hugo软件
4. 你创建好的Hugo站点
5. Github Desktop软件（如果熟练使用Git软件了那么请便）
6. 能加速Github的任意加速器（如果不用加速可以跳过）

### 创建Github仓库

打开[Github官网](https://github.com)，创建一个新的存储库。
...
如果你想，可以点击“main”右边的三个点，点击“Rename branch”，给主分支改一个名字（例如"Origin"），建议将其作为存放源代码的分支。
...
最后你可以在网站主题目录下的`hugo.toml`中更改一下目录的显示数量就行了（就是从H2到H6显示多少的意思）
...
上面的文件，笔者是这样设置的，反正这里的Markdown转换器使用的是MarkUp。只要在模板文件中包含`{{ .TableOfContents }}`就行了。
...
你如果要写css样式，建议可以先在浏览器中打开开发者工具先在网页中试着添加css样式，感觉可以了再通过代码编辑器写进文件里push。
